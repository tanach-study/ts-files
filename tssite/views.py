...
def all(request):
    ...
    for c in talmud_classes:
        ...
        values.append({
            ...
            'segment_sponsor': talmud_sponsors_map[c.seder_sponsor] if c.seder_sponsor is not None else None,
            'segment_sequence': c.seder_sequence if c.seder_sequence is not '' else None,
            'section': c.masechet,
            'section_name': 'Masechet',
            'section_title': c.masechet,
            'section_sponsor': talmud_sponsors_map[c.masechet_sponsor] if c.masechet_sponsor is not None else None,
            'section_sequence': c.masechet_sequence if c.masechet_sequence is not '' else None,
            'unit': c.daf,
            'unit_name': 'Daf',
            'unit_title': c.daf,
            'unit_sponsor': talmud_sponsors_map[c.daf_sponsor] if c.daf_sponsor is not None else None,
