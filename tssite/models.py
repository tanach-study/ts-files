...
masechetot_by_seder = [
    ("Zeraim", (("berachot", "Berachot"),)),
    ("Moed", (("shabbat", "Shabbat"),)),
]
...
def get_class_audio_location(instance, filename):
    ...
    if instance.division == "torah":
        ...
        instance.division == "neviim_rishonim"
        or instance.division == "neviim_aharonim"
        or instance.division == "tere_assar"
    ):
        base = ""
...
class Class(models.Model):
    ...
    audio = models.FileField(
        upload_to=get_class_audio_location,
    ...
    teacher = models.ForeignKey(
        Teacher, on_delete=models.SET_DEFAULT, default=None, null=True
    )
    ...
    def get_location(self):
        ...
        if self.division == "torah":
            toreturn = (
                f"/parasha-study/perakim/{self.section}/{self.unit}?part={self.part}"
            )
        elif (
            self.division == "neviim_rishonim"
            or self.division == "neviim_aharonim"
            or self.division == "tere_assar"
            or self.division == "ketuvim"
        ):
            if self.part:
                toreturn = (
                    f"/tanach-study/perakim/{self.section}/{self.unit}?part={self.part}"
                )
            else:
    ...
    def __str__(self):
        ...
        if self.division == "torah":
            ...
            if self.part:
                ...
                toreturn = (
                    f"Tanach - Sefer {self.section_title}: Perek {self.unit.title()}"
                )
...
def get_teamim_audio_location(instance, filename):
    ...
    if instance.post.division == "torah":
        ...
        instance.post.division == "neviim_rishonim"
        or instance.post.division == "neviim_aharonim"
        or instance.post.division == "tere_assar"
    ):
        base = ""
...
class Teamim(models.Model):
    ...
    audio = models.FileField(
        upload_to=get_teamim_audio_location,
        default=None,
        null=True,
        blank=True,
        max_length=500,
    ...
    post = models.ForeignKey(
        Class, on_delete=models.SET_DEFAULT, default=None, null=True, blank=True
    )
...
class TalmudStudy(models.Model):
    ...
    seder_sponsor = models.ForeignKey(
        TalmudSponsor,
        related_name="+",
    ...
    masechet_sponsor = models.ForeignKey(
        TalmudSponsor,
        related_name="+",
        on_delete=models.SET_DEFAULT,
        default=None,
    ...
    audio = models.FileField(
        upload_to=get_talmud_audio_location,
        validators=[validate_file_extension],
        default=None,
        null=True,
        max_length=500,
    ...
    models.UniqueConstraint(
        fields=["masechet", "daf", "amud"], name="unique_daf_amud_per_masechet"
    )

    def __str__(self):
        return (
            f"Masechet {self.masechet.title()} Daf {self.daf} with {str(self.teacher)}"
        )
